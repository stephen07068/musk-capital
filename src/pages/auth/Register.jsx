...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const user = await register({ name, email, username, password, confirmPassword });
    ...
    <div className="min-h-screen bg-[#07090D] flex items-center justify-center py-20 px-4">
      ...
      <div className="w-full max-w-md relative">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[#9CA3AF] mt-2">Start your investment journey today</p>
        </div>
        ...
        <div className="card-dark p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Full Name */}
            <div>
              <label htmlFor="reg-name" className="block text-xs font-semibold text-[#9CA3AF] mb-2">Full Name</label>
            ...
            {/* Email Address */}
            <div>
              <label htmlFor="reg-email" className="block text-xs font-semibold text-[#9CA3AF] mb-2">Email Address</label>
              <div className="relative">
            ...
            {/* Username */}
            <div>
              <label htmlFor="reg-username" className="block text-xs font-semibold text-[#9CA3AF] mb-2">Username</label>
              <div className="relative">
                <AtSign size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6B7280]" />
                <input
                  id="reg-username"
                  name="username"
                  type="text"
            ...
            {/* Password */}
            <div>
              <label htmlFor="reg-password" className="block text-xs font-semibold text-[#9CA3AF] mb-2">Password</label>
              <div className="relative">
            ...
            {/* Confirm Password */}
            <div>
              <label htmlFor="reg-confirm-password" className="block text-xs font-semibold text-[#9CA3AF] mb-2">Confirm Password</label>
              <div className="relative">
                <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6B7280]" />
                <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 gold-gradient text-black font-bold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-70 mt-2"
            >
              {loading ? (
          ...
          <p className="text-center text-sm text-[#9CA3AF] mt-6">
            ...
            <Link to="/login" className="text-[#D4AF37] font-semibold hover:text-[#F0D060] transition-colors">Sign In</Link>
          </p>
        </div>

        <p className="text-center text-xs text-[#6B7280] mt-4">
          By registering you agree to our{' '}
          <Link to="/terms" className="text-[#9CA3AF] hover:text-white">Terms of Service</Link>{' '}
          and{' '}
          <Link to="/privacy" className="text-[#9CA3AF] hover:text-white">Privacy Policy</Link>
        </p>
      </div>
    </div>
  );
