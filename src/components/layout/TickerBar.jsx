...
export default function TickerBar({ isFixed = false }) {
  ...
  const doubled = [...liveMarketData, ...liveMarketData]; // duplicate for seamless loop
    ...
    <div className={`w-full bg-[#0D0F14] border-b border-[#1F2937] overflow-hidden py-2 ${isFixed ? 'fixed top-0 left-0 right-0 z-[60]' : 'relative z-0'}`}>
      <div className="flex items-center gap-0 ticker-scroll whitespace-nowrap" style={{ width: 'max-content' }}>
        {doubled.map((item, idx) => (
          <div key={`${item.symbol}-${idx}`} className="flex items-center gap-2 px-6 border-r border-[#1F2937]">
            ...
            <span className={`flex items-center gap-0.5 text-xs font-medium ${item.changePercent >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
              {item.changePercent >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />}
              {item.changePercent >= 0 ? '+' : ''}{item.changePercent.toFixed(2)}%
            </span>
            {idx === liveMarketData.length - 1 && (
